...
- [ ] features/items-core и features/progression-core существуют,
      ...
      байт-идентично до/после (SHA скринов + полный ctest зелёный +
      wasm-линк обоих пресетов).
...
- (1) Шов карточки «ядро Unity-тестов → items-core» ПЕРЕИНТЕРПРЕТИРОВАН:
  ...
  инвестиция при второй игре. ДЕФОЛТ: принять переинтерпретацию.
- (2) Игровой уголок items остаётся src/features/items/ (reason_tags.h
  ...
  ДЕФОЛТ: оставить src/features/items/.
- (3) «SHA скринов» из Done-when: добавлен гейт капчи канонического
  кадра до/после (devapi capture.frame + SHA-сравнение); при флейке
  по GPU/драйверу — списать явно с обоснованием. ДЕФОЛТ: гейт есть.
