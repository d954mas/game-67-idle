...
- [x] Миграция screen-флагов НЕ нужна — решение лида 2026-07-07: флаги ставит
      руками, когда нужен экспорт (скрипт остаётся опцией для массовой
      разметки, обязательным шагом не является).
- [ ] Лид: приёмка фичи в UI.
...
- 2026-07-07: Лид: миграция не нужна, screen-флаги вручную по надобности; остаётся только приёмка UI
- 2026-07-07: Лид одобрил agy-smoke для паков: снять codex-only гейт (packPreview + pack-ветка generate), 1 smoke-лист на gemini; главный риск — адгезия сетки; не держит -> вернуть гейт с evidence. UX-фиксы по живому дебагу лида закоммичены (дефолт тумблера, авто-vary, slot-warning, видимость превью, спойлеры)
- 2026-07-07: agy-smoke ПРОЙДЕН, паки открыты на gemini: гейт сужен до запрета "both" (двойная цена + ломает resume-идентичность), диспатч по recipe.engine, meta.pack.engine на каждом листе, engine-aware UI-подписи. Живьём (potion 2×2, 3 листа): сетку и плоский #FF00FF agy держит, но системно рисует линии-разделители -> slice REJECT 5/4 (2/2); лечится запретом линий в промпте карты -> OK 4/4 с первой попытки. Правило в README Pack mode + throughput-and-handoff.md; 711/711 тестов (+1 gemini-диспатч)
