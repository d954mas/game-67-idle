...
- 2026-07-08 (консервация, лид сворачивается по токенам): МИНИ-РАУНД
  ...
  каждой игры, которая отгрузит эти пиксели в паке (README ассетов —
  source-side док, в билд НЕ шипается, обязательство НЕ покрывает;
  наследования атрибуции в new_game.mjs нет) — ЖДЁТ решения лида.
  Рекомендация deep-ревью: для demo-плейсхолдеров шаблона ЗАМЕНИТЬ
  на CC0-иконки (у game-icons.net есть CC0-авторы; выбранная тройка
  — именно CC BY) — снимает обязательство целиком; иначе правило
  «игра с demo-иконками обязана credits / реальные игры заменяют»; (б) SVG-источник — РАТИФИЦИРОВАНО лидом 2026-07-08 («я в
