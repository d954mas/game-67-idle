...
- Пак-рецепт иконок должен быть ДАННЫМИ (атлас-манифест JSON:
  atlas → [{region, file}]), не кодом: читателей три — упаковщик,
  редактор (превью: пара → исходник), сборочная сверка (множество
  пар из items.json ⊆ множество пар манифеста = unresolved-symbol
  чек, переименование ловится сборкой). build_packs.c шелл-ассетов
  (шрифт/шейдеры) остаётся кодом — их редактор не показывает.
...
- 2026-07-08: ФАЗА 1 ИСПОЛНЕНА, 4 коммита: спека 3789e24bc (2
  ...
  → null не тестирован (звучит здраво). ОСТАЛОСЬ ЛИДУ: приёмка
  глазами — node ai_studio/studio_shell/server.mjs →
  http://127.0.0.1:8765/items. Дальше: мини-раунд иконок (What,
  решения 2026-07-08), фаза 2 — с write-слоем и реальной игрой.
