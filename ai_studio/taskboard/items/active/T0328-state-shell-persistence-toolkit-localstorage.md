...
- E3/ночь 2026-07-07, ДЕФОЛТ ПРИМЕНЁН, ждёт ратификации лида:
  курсор game.events.tail переведён на offset-семантику (seq >= since,
  next_seq = last+1) — спека-буквальная `seq > since` с дефолтом 0
  МОЛЧА теряла первое событие сессии (seq стартует с 0; доказано
  живьём исполнителем). Потребителей старой семантики не было (E4
  курсор не использует). Амендмент в build_spec_e3 §E3.3/§E3.9.
- E4/ночь 2026-07-07, ДЕФОЛТ ПРИМЕНЁН, ждёт подтверждения лида:
  семантика потери аналитики при капе ПРОТИВОПОЛОЖНА по платформам:
  native = keep-oldest-stop (8МБ кап, теряется хвост сессии), web-ring
  = keep-newest-roll (256КБ, теряется начало). Дефолт: оставлено,
  расхождение задокументировано в спеке E4 + GAME_ANALYTICS_MAX_BYTES=0
  (без капа) как дешёвый override. Связанный под-вопрос: писать ВСЕ
  события (raw-firehose, дефолт) или opt-in селективность (§4 дизайна
  читается двояко) — firehose быстрее набивает кап.
- A5/ночь 2026-07-07, ДЕФОЛТ ПРИМЕНЁН, ждёт подтверждения лида:
  ...
  фрагменты). Напряжение: state doc §8 говорит «агрегат = features-map»,
  а on-disk features-map включает orphan verbatim (§14 п.16) — DevAPI
  слеп ровно к данным, которые §14 п.16 защищает (отладка ретеншена
  сирот через бота невозможна). Реверс дешёвый: + s_orphans через
  новый геттер. Детали: build_spec_a5_2026-07-07.md, Q1.
