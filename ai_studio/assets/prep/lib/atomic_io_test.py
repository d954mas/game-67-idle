...
from ai_studio.assets.prep.lib.atomic_io import copy_file_atomic, save_image_atomic, write_json_atomic, write_text_atomic
...
class AtomicIoTest(unittest.TestCase):
    ...
    def test_copy_file_atomic_keeps_existing_file_on_copy_failure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("ai_studio.assets.prep.lib.atomic_io.shutil.copyfile", failing_copyfile):
                with self.assertRaises(RuntimeError):
