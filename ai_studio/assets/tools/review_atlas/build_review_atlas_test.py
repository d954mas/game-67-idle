...
SCRIPT = ROOT / "ai_studio/assets/tools/review_atlas/build_review_atlas.py"


def load_builder_module():
    spec = importlib.util.spec_from_file_location("build_review_atlas_module", SCRIPT)
...
class BuildReviewAtlasTest(unittest.TestCase):
    ...
    def test_packs_assets_with_extruded_padding_and_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            write_png(root / "assets/prepared/panel.png", color=(255, 0, 0, 255))
            ...
            self.assertEqual(pack["schema"], "game.review_atlas")
    ...
    def test_packs_separate_pack_groups(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_png(root / "assets/prepared/icon.png", size=(5, 5), color=(30, 60, 240, 255))
            manifest = root / "manifest.json"
            manifest.write_text(
                json.dumps(
                    {
                        ...
                        "version": 1,
                        "assets": [
                            asset("panel", "assets/prepared/panel.png", pack_group="ui_panels"),
                            asset("icon", "assets/prepared/icon.png", kind="icon", pack_group="ui_icons", original_size=[5, 5], trim_rect=[0, 0, 5, 5], atlas_policy={**asset("x", "x")["atlas_policy"], "allow_rotation": True}),
                        ],
                    }
                ),
                encoding="utf-8",
            ...
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            pack = json.loads((root / "packed/review-atlas.json").read_text(encoding="utf-8"))
    ...
    def test_label_review_lists_alias_ids_on_physical_asset(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_png(root / "assets/prepared/button_base.png")
            write_png(root / "assets/prepared/button_primary.png")
            write_png(root / "assets/prepared/button_secondary.png")
            manifest = root / "manifest.json"
            manifest.write_text(
                json.dumps(
                    {
                        ...
                        "assets": [
                            asset("button_base", "assets/prepared/button_base.png"),
                            asset("button_secondary", "assets/prepared/button_secondary.png", alias_of="button_base"),
                            asset("button_primary", "assets/prepared/button_primary.png", alias_of="button_base"),
                        ],
                    }
                ),
    ...
    def test_label_review_marks_manifest_as_review_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_png(root / "assets/prepared/panel.png")
            manifest = root / "manifest.json"
            manifest.write_text(json.dumps({"schema": "game.asset_manifest", "version": 1, "assets": [asset("panel", "assets/prepared/panel.png")]}), encoding="utf-8")
            result = run_pack(
                root,
                "--asset-manifest",
                "manifest.json",
            ...
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            pack = json.loads((root / "packed/atlas.json").read_text(encoding="utf-8"))
            self.assertEqual(pack["purpose"], "review_validation_atlas")
    ...
    def test_profile_output_keeps_review_manifest_stable(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(profile["schema"], "game.review_atlas_profile")
    ...
    def test_label_review_can_use_right_side_free_space_for_small_assets(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            write_png(root / "assets/prepared/tiny_icon.png", size=(12, 12), color=(40, 120, 220, 255))
            manifest = root / "manifest.json"
            manifest.write_text(
                json.dumps(
                    {
    ...
    def test_fails_when_asset_exceeds_max_size(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            write_png(root / "assets/prepared/huge.png", size=(80, 80))
            manifest = root / "manifest.json"
            manifest.write_text(
                json.dumps({"schema": "game.asset_manifest", "version": 1, "assets": [asset("huge", "assets/prepared/huge.png", original_size=[80, 80], trim_rect=[0, 0, 80, 80])]}),
                encoding="utf-8",
            )
