...
function run(args, cwd) {
  return spawnSync(process.execPath, [join(root, "ai_studio/assets/workflow/art_jobs/validate_art_job.mjs"), ...args], {
    cwd,
    encoding: "utf8",
    ...
    required_asset_groups: [
      {
        ...
        kind: "slice9",
        states: ["default"],
        content_policy: "game text inside content area",
...
test("strict mode rejects missing source sheets and crops", (t) => {
  ...
  assert.match(result.stdout, /strict mode requires crop plan sources/);
});
...
test("strict mode rejects prepared assets manifest missing a crop asset", (t) => {
  ...
  const result = run(["--job", job, "--strict"], dir);
  assert.equal(result.status, 1);
  assert.match(result.stdout, /prepared assets manifest missing asset for crop resource_icon/);
});
...
test("strict mode rejects prepared assets manifest pointing at another crop plan", (t) => {
  ...
  const result = run(["--job", job, "--strict"], dir);
  assert.equal(result.status, 1);
  assert.match(result.stdout, /prepared assets manifest crop_plan should match expected_outputs.crop_plan/);
});
...
test("final-art mode rejects required decor source family without prepared decor assets", (t) => {
  const dir = tempDir(t);
  const { job } = writeStrictValidJob(dir);
  const jobData = JSON.parse(readFileSync(join(dir, job), "utf8"));
  ...
  const result = run(["--job", job, "--final-art"], dir);
  assert.equal(result.status, 1);
  assert.match(result.stdout, /required source family ui decor overlay sheet needs prepared decor overlay/);
});

test("final-art mode allows explicit partial prepared slice to defer source families", (t) => {
  const dir = tempDir(t);
  const { job } = writeStrictValidJob(dir);
  const jobData = JSON.parse(readFileSync(join(dir, job), "utf8"));
  jobData.generation_contract.source_families.push("ui decor overlay sheet");
  ...
  jobData.expected_outputs.delivery_scope = {
    mode: "partial_prepared_slice",
    included_source_families: ["blank UI kit sheet", "isolated icon sheet"],
    deferred_source_families: ["ui decor overlay sheet"],
    reason: "This proof validates slice9 bases and icons before decor overlays are cut into prepared assets.",
  };
  writeFileSync(join(dir, job), `${JSON.stringify(jobData, null, 2)}\n`, "utf8");

  const result = run(["--job", job, "--final-art"], dir);
  assert.equal(result.status, 0, result.stdout + result.stderr);
  assert.match(result.stdout, /partial-prepared-slice-valid/);
  assert.match(result.stdout, /scope: partial_prepared_slice/);
