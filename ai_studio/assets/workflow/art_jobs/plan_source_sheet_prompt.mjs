...
function usage() {
  console.log(`usage:
  node ai_studio/assets/workflow/art_jobs/plan_source_sheet_prompt.mjs --job <art-job.json> --source-family "<family>" --output <packet.md> [--json-output <packet.json>] [--intake-audit <audit.json>] [--key-color #00ff00] [--allow-chroma-after-preserve-risk] [--force]
...
function layoutRowsForFamily(sourceFamily, relevantGroups) {
  ...
  return [
    {
      id: "isolated_assets",
      purpose: "all requested reusable prepared assets for this source family",
      item_policy: "one isolated component per slot with clear gutters and no composed gameplay screen",
      asset_group_ids: groupIds,
    },
  ];
}
    ...
    .map((row) => {
      ...
      return {
        ...
        item_policy: hasText(row.item_policy)
          ...
          : "one isolated component per slot with clear gutters and no composed gameplay screen",
        slots,
      };
    });
}
