...
const preparedDir = `${projectDir}/art/prepared/${args.id}`;
...
const artJob = {
  ...
  candidate_policy: {
    ...
    reject_to_tmp: true,
      accept_one_or_two_for_prepared_delivery: true
  },
  ...
  required_asset_groups: [
    {
      ...
      states: ["default"],
      content_policy: "game text/icons only inside content safe area",
    ...
    },
    {
      ...
      content_policy: "game label/icon composed over blank background",
  ...
  generation_contract: {
    ...
    final_asset_policy: {
      ...
      notes: "Code may cut, validate, and compose. Code-generated/programmer art is not acceptable as final generated UI unless an explicit exception is recorded."
    },
    source_family_roles: {
  ...
  expected_outputs: {
    ...
    generation_records: [],
    crop_plan: cropPlanPath,
    prepared_assets: preparedAssetsPath,
    prepared_dir: preparedDir,
    native_evidence: []
  },
  game_composition: {
    buttons: "slice9 background plus game text/icon",
    cards: "slice9 card background plus character sprite and game state",
    board: "frame, tile, slot, highlight, and effect pieces composed in game",
  ...
  commands: {
    plan_source_sheet_prompt: `node ai_studio/assets/workflow/art_jobs/plan_source_sheet_prompt.mjs --job ${artRequestPath} --source-family "<source family>" --output ${projectDir}/art/prompts/<source-id>-prompt.md --json-output ${projectDir}/art/prompts/<source-id>-prompt.json`,
    new_generation_record: `node ai_studio/assets/workflow/art_jobs/new_generation_record.mjs --id <source-id> --project-dir ${projectDir} --source-family "<source family>" --source-family-role "<source family role>" --accepted-source <path> --provider <provider> --model <model-or-workflow> --workflow-path <workflow.json> --prompt-packet ${projectDir}/art/prompts/<source-id>-prompt.json --seed <seed> --prompt "<prompt>" --negative-prompt "<negative prompt>"`,
    slice_assets: "fill after source sheets are accepted",
    validate_draft: `node ai_studio/assets/workflow/art_jobs/validate_art_job.mjs --job ${artRequestPath}`,
    validate_strict: `node ai_studio/assets/workflow/art_jobs/validate_art_job.mjs --job ${artRequestPath} --strict`,
    validate_final_art: `node ai_studio/assets/workflow/art_jobs/validate_art_job.mjs --job ${artRequestPath} --final-art`,
    storage_intake: "fill after prepared assets are accepted for storage or project use",
...
writeJson(cropPlanPath, cropPlan, args.dryRun);
writeJson(preparedAssetsPath, preparedAssets, args.dryRun);
