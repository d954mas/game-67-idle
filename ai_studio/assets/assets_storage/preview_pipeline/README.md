...
gallery UI and not asset catalog ownership.
...
node ai_studio/assets/assets_storage/preview_pipeline/render_library_previews.mjs --pack survival-kit
node ai_studio/assets/assets_storage/preview_pipeline/render_library_previews.mjs --all --limit 50
```

The command reads asset records from `../okf_catalog/`, finds GLB files,
skips existing library previews unless `--force` is passed, and runs Blender
headlessly in chunks.
...
- Source intake, licensing, conversion, and pack building still live in legacy
  `tools/assets/` until those modules are reviewed separately.

## Files

- `preview_cache.mjs`: normal source-aware preview cache refresh used by Asset
  Viewer.
