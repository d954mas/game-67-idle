...
import { listRegisteredTemplates, registerTemplateAssetSource, templateRegistryPath } from "../templates_registry.mjs";
...
test("registerTemplateAssetSource creates and lists a template asset source", (t) => {
  ...
  assert.equal(templateRegistryPath(root), "ai_studio/assets/assets_storage/source_registry/templates.json");
  assert.deepEqual(listRegisteredTemplates(root), [
    {
      id: "mobile-template",
  ...
  const parsed = JSON.parse(readFileSync(join(root, "ai_studio", "assets", "assets_storage", "source_registry", "templates.json"), "utf8"));
