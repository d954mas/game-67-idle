...
It owns file-level added/changed/deleted detection. It does not own catalog
...
Library sources track:

- `catalog/**/*.md`
- asset files under `files/`
- license files under `files/`

Template/game/local sources track:
...
`../preview_pipeline/` and the Asset Viewer `Refresh previews` action.

## Consumers

`../asset_index/` uses this module during `refreshAssetIndex()`:
