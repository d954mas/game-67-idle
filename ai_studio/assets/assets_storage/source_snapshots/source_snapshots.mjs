...
function isTrackedFile(source, path) {
  ...
  if (source.mode !== "library" && manifestExt.has(ext)) return true;
  return /(^|[\\/])licen[cs]e(\.|$)/i.test(path);
}

export function sourceSnapshotPath(root, source) {
  return join(root, "tmp", "ai_studio", "assets", "source_snapshots", `${safeSlug(source.id)}.json`);
}
...
export function buildSourceSnapshot(root, source) {
  ...
  const parts = [sourceKey(source), source.mode, source.path];
  let totalSize = 0;
  let maxMtimeMs = 0;
