...
test("registerLibraryAssetSource creates and lists a library asset source", (t) => {
  ...
  assert.equal(libraryRegistryPath(root), "ai_studio/assets/backlog/storage/sources/libraries.json");
  ...
  const parsed = JSON.parse(readFileSync(join(root, "ai_studio", "assets", "backlog", "storage", "sources", "libraries.json"), "utf8"));
...
test("listRegisteredLibraries accepts UTF-8 BOM registry files", (t) => {
  const root = tempRoot();
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const path = join(root, "ai_studio", "assets", "backlog", "storage", "sources", "libraries.json");
