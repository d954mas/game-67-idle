...
//
//   node ai_studio/assets/backlog/storage/previews/render_library_previews.mjs --source polypizza
//   node ai_studio/assets/backlog/storage/previews/render_library_previews.mjs --pack survival-kit --png
//   node ai_studio/assets/backlog/storage/previews/render_library_previews.mjs --all --limit 50
//
...
async function main() {
  ...
  const { records } = await scanPackManifestSource(a.library);
  const modelRecords = records.filter((r) => r.kind === "model");
  let targets = modelRecords.filter((r) => {
  ...
  for (const r of targets) {
    const glb = r.modelPath || glbIn(r.filesDir);
    if (!glb) { missing += 1; continue; }
    const prevDir = r.preview ? dirname(r.preview) : join(dirname(r.metadataPath), "previews", r.asset_id);
  ...
  for (let c = 0; c < items.length; c += CHUNK) {
    ...
    for (const it of chunk) {
      ...
      const record = targets.find((target) => target.asset_id === id);
      const prevDir = record?.preview ? dirname(record.preview) : join(dirname(record?.metadataPath || a.library), "previews", id);
