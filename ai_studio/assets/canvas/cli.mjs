...
//   node ai_studio/assets/canvas/cli.mjs alpha <id> --element <eid> [--method auto|matte|corridorkey|vitmatte|birefnet] [--regions r1,r2]
//   node ai_studio/assets/canvas/cli.mjs alpha <id> --elements e1,e2 [--method auto|matte|corridorkey|vitmatte|birefnet]   (batch; one undo)
...
//   node ai_studio/assets/canvas/cli.mjs recipe-pack-generate <id> --group g [--run <groupId>] [--sheet <slug>]   (T0332 v2: the pack branch of generateFromRecipe — real engine spawns (codex or agy, per the card's recipe.engine), one PER SHEET, sequential; each finished sheet mints under its own short commit; --run resumes into an existing pack run group, skipping sheets whose axes already landed; --sheet force-regenerates exactly one sheet into that group; prints per-sheet ok/failed/skipped lines + the final recipe.last_run summary)
...
function usage() {
  ...
  alpha <id> --element <eid> [--method auto|matte|corridorkey|vitmatte|birefnet] [--regions r1,r2]   (alpha-cutout the element; auto routes, matte forces key_matte, corridorkey = neural green-screen matte for soft glow (green native, magenta via hue180 shim; regions composite the whole-frame CK result into the requested regions; ~15s GPU); vitmatte = neural THIN detail (spider-web/mesh/fur/hair) on a green/magenta key + 2nd-choice glow, its OWN GPU venv, ~1-3s, whole-element only; birefnet = SOD cutout for ANY/unknown background, NO key, shared repo venv CPU ~10-30s, whole-element only; one undo)
  alpha <id> --elements e1,e2 [--method auto|matte|corridorkey|vitmatte|birefnet]   (batch: 2+ images keyed into ONE journal entry/undo; no --regions with a batch; corridorkey/vitmatte/birefnet each pay their per-image cost — the batch runs sequentially)
