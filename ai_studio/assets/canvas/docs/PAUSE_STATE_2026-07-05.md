...
1. **T0265 — карточка анимации, инкремент 1** — ПОСТРОЕН 2026-07-06 на
   ветке anim-card-t0265 (см. лог T0265): опы+API/CLI+flipbook-плеер+
   инспектор, сьют 646/646, ревью закрыто. Ждёт живого прогона лида
   (ComfyUI вручную) и merge в master. Инкремент 2 = режим-сцена с
   таймлайном (обрезка/удаление/fps/режимы). Инкремент 3 = FLF (нужны
   FLF-workflow JSON — T0266).
