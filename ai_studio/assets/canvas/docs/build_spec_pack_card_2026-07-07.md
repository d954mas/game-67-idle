...
- 2026-07-07 post-review пакет (2× Opus ревью: блокеров 0) + решение лида
  «both на паках»: (лид: «почему если я выбрал значит мне это нужно» — цена
  = решение лида, отказ был патернализмом). Реализовано: engine "both" на
  паках легален — каждая джоба фанится на оба движка (2× вызовов), листы
  именуются "<job> codex"/"<job> agy", идентичность листа ВЕЗДЕ = пара
  (sheet_axes, engine) — резюм-дедуп и --sheet-replace per-engine; легаси
  листы без meta.pack.engine считаются codex (фактическая правда). Новое
  поле meta.pack.job = голое имя джобы экспандера (стабильный ключ --sheet
  под суффиксованным display-именем; UI Regenerate использует его). Смена
  движка карты между раном и резюмом генерит версии нового движка РЯДОМ со
  старыми (дёшёвое сравнение, не двойной биллинг — skip-строки печатаются).
  Ревью-фиксы: README tool_runs абзац (учил старую форму), сброс залипшего
  pack-превью при патче карты, +4 пиннинг-теста (both fan-out, resume по
  (axes,engine), flip+legacy, preview на gemini/both) + легаси-supersample
  тест + per-engine снапшоты в both-тесте одиночки. 715/715.
