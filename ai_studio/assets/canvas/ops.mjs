...
// recipe/style/pack_run (the same objects the old filter excluded, plus a pack run group —
...
export function migrateScreenFlags(root, { projectId } = {}) {
  ...
  const nextGroups = groupsOf(before).map((group) => {
    ...
    if (group.recipe || group.style || group.pack_run) return group; // workshop/run object
