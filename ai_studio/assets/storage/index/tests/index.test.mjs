...
function tempLibrary() {
  ...
  const packDir = join(library, "packs", "furniture");
  mkdirSync(join(packDir, "files", "kenney__red-sofa__cc0"), { recursive: true });
  mkdirSync(join(packDir, "files", "kenney__blue-chair__cc0"), { recursive: true });
  mkdirSync(join(packDir, "previews", "kenney__red-sofa__cc0"), { recursive: true });
  mkdirSync(join(packDir, "previews", "kenney__blue-chair__cc0"), { recursive: true });
  writeFileSync(join(packDir, "files", "kenney__red-sofa__cc0", "sofa.glb"), "glb", "utf8");
  writeFileSync(join(packDir, "files", "kenney__blue-chair__cc0", "chair.glb"), "glb", "utf8");
  writeFileSync(join(packDir, "previews", "kenney__red-sofa__cc0", "preview.webp"), "webp", "utf8");
  writeFileSync(join(packDir, "previews", "kenney__blue-chair__cc0", "preview.webp"), "webp", "utf8");
  ...
  writeJsonl(join(packDir, "assets.jsonl"), [
    {
      ...
      kind: "model",
      resource: "files/kenney__red-sofa__cc0/sofa.glb",
      preview: "previews/kenney__red-sofa__cc0/preview.webp",
      tags: ["sofa", "furniture"],
    },
    {
      ...
      kind: "model",
      resource: "files/kenney__blue-chair__cc0/chair.glb",
      preview: "previews/kenney__blue-chair__cc0/preview.webp",
...
function source(library) {
  return { id: "global-library", type: "library", label: "All Assets", path: library };
}
...
test("rebuildAssetIndex persists pack manifest assets and pack summaries", async (t) => {
...
test("refreshAssetIndex skips full rebuild when the source signature is unchanged", async (t) => {
  ...
  const packDir = join(library, "packs", "furniture");
  mkdirSync(join(packDir, "files", "kenney__green-table__cc0"), { recursive: true });
  writeFileSync(join(packDir, "files", "kenney__green-table__cc0", "table.glb"), "glb", "utf8");
  writeJsonl(join(packDir, "assets.jsonl"), [
    {
      asset_id: "kenney__red-sofa__cc0",
      title: "Red Sofa",
    ...
    },
    {
      ...
      kind: "model",
      resource: "files/kenney__green-table__cc0/table.glb",
...
test("queryIndexedAssets supports paging, search, filters, facets, and model lookup", async (t) => {
  ...
  assert.equal(page.assets[0].thumb, "lib/packs/furniture/previews/kenney__red-sofa__cc0/preview.webp");
  ...
  assert.equal(model.model, "lib/packs/furniture/files/kenney__red-sofa__cc0/sofa.glb");
});
...
test("queryIndexedAssets resolves secondary pack memberships", async (t) => {
  const { root, library } = tempLibrary();
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const showcaseDir = join(library, "packs", "showcase");
  mkdirSync(showcaseDir, { recursive: true });
  writeJson(join(showcaseDir, "pack.json"), {
    pack: "showcase",
    title: "Showcase Pack",
    source: "Kenney",
    kind: "model",
  ...
  writeJsonl(join(showcaseDir, "assets.jsonl"), []);
  writeJsonl(join(library, "packs", "furniture", "assets.jsonl"), [
    {
      asset_id: "kenney__red-sofa__cc0",
      title: "Red Sofa",
      description: "Cozy red sofa for room scenes.",
...
test("rebuildAssetIndex keeps unregistered files visible for manifest library sources", async (t) => {
  const { root, library } = tempLibrary();
  t.after(() => rmSync(root, { recursive: true, force: true }));
  mkdirSync(join(library, "packs", "furniture", "files", "loose-asset"), { recursive: true });
  writeFileSync(join(library, "packs", "furniture", "files", "loose-asset", "loose-crate.glb"), "glb", "utf8");
...
test("rebuildAssetIndex prefers pack manifests for library sources", async (t) => {
  const root = mkdtempSync(join(tmpdir(), "asset-index-library-manifest-"));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const library = join(root, "library");
  const packDir = join(library, "packs", "starter-props");
  mkdirSync(join(packDir, "files"), { recursive: true });
  writeFileSync(join(packDir, "files", "crate.glb"), "glb", "utf8");
  writeFileSync(join(packDir, "pack.json"), JSON.stringify({
    pack: "starter-props",
  ...
  assert.equal(page.total, 1);
  assert.equal(page.assets[0].id, "starter__crate__cc0");
  assert.equal(page.assets[0].origin, "mine");
});
...
test("rebuildAssetIndex keeps unregistered files visible for manifest sources", async (t) => {
  ...
  assert.equal(page.assets[0].origin, "unregistered");
  assert.equal(page.assets[0].license, "unknown");
  assert.equal(page.assets[0].previewStatus, "clean");
});
