...
import { decideLicense } from "../license/registry.mjs";
import {
  buildSourceSnapshot,
  diffSourceSnapshots,
  readSourceSnapshot,
...
async function readRegisteredSourceData(root, source, snapshot = null) {
  if (hasPackManifestSource(source.path)) {
    return scanPackManifestSource(source.path);
  }
  return { records: [], packs: [] };
}

function isCoveredDiscoveredRecord(record, covered) {
  return isCoveredPath(record.metadataPath || "", covered);
}
...
export async function rebuildAssetIndex(root, source, options = {}) {
  ...
  mark("snapshot");
  const sourceData = await readRegisteredSourceData(root, source, snapshot);
  mark("scan");
  ...
  try {
    ...
    for (const record of records) {
      ...
      const cachedPreview = record.preview
        ? ""
