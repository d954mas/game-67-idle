...
(() => {
  ...
  const OC = { mine: "#7dd3fc", ai: "#f0abfc", sourced: "#86efac", unregistered: "#fb7185", unknown: "#9ca3af" };
  ...
  async function reindexSelectedSource() {
    ...
    setViewerBusy(source.type === "library" ? `Reindexing ${source.label}...` : `Refreshing ${source.label}...`);
    try {
  ...
  function assetCard(a) {
    ...
    let foot = "";
    const isUnregistered = a.origin === "unregistered";
    const creditRequired = a.attributionRequired === "true";
    const noticeRequired = a.noticeRequired === "true";
    const warning = isUnregistered ? '<span class="warnchip">not in metadata</span>' : "";
    const credit = creditRequired ? '<span class="creditchip">credit required</span>' : "";
    const notice = !creditRequired && noticeRequired ? '<span class="creditchip">notice required</span>' : "";
    const pathLine = isUnregistered && (a.relpath || a.id) ? '<div class="asset-path" title="' + esc(a.relpath || a.id) + '">' + esc(a.relpath || a.id) + "</div>" : "";
    ...
    return '<div class="card' + (picked.has(a.id) ? " sel" : "") + (isUnregistered ? " unregistered" : "") + '" data-asset="' + esc(a.id) + '">' + t +
      '<div class="meta"><div class="name">' + esc(a.name) + "</div>" + pathLine + '<div class="row">' +
      ...
      '<span class="k">' + esc(a.kind) + "</span>" + warning + credit + notice + "</div>" + packChips(a) + "</div>" + foot + "</div>";
  }
  ...
  function openAsset(id) {
    ...
    const ins = "<h3>" + esc(a.name) + "</h3>" +
      ...
      kv("license kind", esc(a.licenseKind)) +
      kv("attribution", a.attributionRequired === "true" ? esc(a.creditText || ((a.authorVendor || "credit required") + (a.sourcePage ? " - " + a.sourcePage : ""))) : "") +
      kv("notice", a.noticeRequired === "true" ? esc(a.creditText || a.authorVendor || "license notice required") : "") +
  ...
  function setCount(shown, total, rendered = shown) {
    const c = $("count");
    if (!c) return;
    c.textContent = rendered < shown ? rendered + " / " + shown + " visible (" + total + " total)" : shown + " / " + total;
  }

  function disconnectAutoLoad() {
    if (loadObserver) {
  ...
  function renderAutoMore({ listLength = 0, loaded = 0, total = 0, pack = "", lazy = false } = {}) {
    const more = $("gridMore");
    if (!more) return;
    const hasMore = lazy ? loaded < total : listLength > renderCap;
    if (!hasMore) {
      more.innerHTML = "";
      disconnectAutoLoad();
      return;
    }
    const next = lazy ? Math.min(RENDER_STEP, total - loaded) : Math.min(RENDER_STEP, listLength - renderCap);
    more.innerHTML = '<div class="load-sentinel" id="loadSentinel" data-lazy="' + (lazy ? "1" : "0") +
      '" data-pack="' + esc(pack) + '"><span>Loading next ' + next + "</span></div>";
  }
  ...
  function layoutHtml() { return '<div class="layout"><aside class="facets" id="facets"></aside><main class="asset-list"><div id="abar"></div><div class="grid" id="grid"></div><div class="morebar" id="gridMore"></div></main></div>'; }
  ...
  function render() {
    ...
    if (r.view === "pack") {
      ...
      if (OPTS.assetsLazy && !isLoose) renderAutoMore({ loaded: DATA.length, total: lazyTotal, pack: r.arg, lazy: true });
      else renderAutoMore({ listLength: list.length });
    ...
    if (r.view === "all") {
      ...
      $("abar").innerHTML = activeBar();
      $("grid").innerHTML = visible.map(assetCard).join("");
      if (OPTS.assetsLazy) renderAutoMore({ loaded: DATA.length, total: lazyTotal, lazy: true });
      else renderAutoMore({ listLength: list.length });
    ...
    renderAutoMore({ listLength: list.length });
  ...
  function bindAutoLoad() {
    ...
    const loadNext = () => {
      ...
      if (sentinel.dataset.lazy === "1") {
        loadAssetsForScope(sentinel.dataset.pack || "", true)
          .then(render)
          .catch((error) => {
  ...
  function shell() {
    ...
    const refreshButton = DYNAMIC && selectedSource && selectedSource.available ? '<button class="ghost compact" id="refreshSource" type="button" title="Refresh selected source index">Reindex</button>' : "";
    const previewButton = DYNAMIC && selectedSource && selectedSource.available ? '<button class="ghost compact" id="refreshPreviews" type="button" title="Refresh missing or stale previews">Previews</button>' : "";
    ...
    const sourceControl = DYNAMIC ? '<div class="sourcebar"><label class="sourcepick"><span>Source</span><select id="sourceSelect">' +
      sourceOptions("library", "Libraries") + sourceOptions("template", "Templates") + sourceOptions("game", "Games") +
      "</select></label>" + refreshButton + previewButton + "</div>" : "";
    ROOT.innerHTML =
      '<header class="viewer-header"><div class="viewer-titlebar"><div class="titleblock"><h1>' + esc(OPTS.title || "Asset library") +
      '</h1><span class="source-status" id="sourceStatus" title="' + esc(OPTS.sourcePath || "") + '">' + esc(OPTS.sourcePath || "") + "</span></div>" +
      sourceControl + '</div><div class="viewer-toolbar">' + tabs +
      '<input type="search" id="q" placeholder="поиск… sofa, fantasy, kenney">' +
      '<select id="sort" aria-label="Sort assets"><option value="name">Name</option><option value="count">Count</option><option value="origin">Origin</option><option value="random">Random</option></select>' +
      '<span class="count" id="count"></span></div></header><div id="app"></div>' +
