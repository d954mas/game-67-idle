...
- Game tools/scripts: `tools/<game-id>/` only; reset removes it.
...
- Workflow: `AI_PIPELINE.md`; temp output: `tmp/`.

## Invariants

- Native PC is playable/automation harness unless lead approves otherwise.
...
- Profiling is automatic; review a session with `node tools/ai.mjs status`.
...
- Named refs/mismatches need the smallest honest Reference Digest before implementation.
