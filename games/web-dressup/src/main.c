...
static bool s_capture_show; /* after load: begin Fake Show and advance to podium for evidence */
...
static void frame(void) {
    ...
    nt_resource_step();
    nt_material_step();
    const bool playable_shell_ready = ui_runtime_ready();
    ...
    /* Soft fashion-room clear (matches blush UI / boudoir stage). */
    nt_gfx_begin_pass(&(nt_pass_desc_t){.clear_color = {0.18F, 0.10F, 0.14F, 1.0F}, .clear_depth = 1.0F});
    (void)s_frame_ubo;
    (void)s_text_material;
    (void)s_font_resource;
    (void)s_font;
    ...
    s_frame_count += 1;
    if (s_capture && s_capture_show && s_frame_count == 8 && playable_shell_ready) {
        /* Evidence path: podium with rival layered looks drawn. */
        dress_room_begin_show();
        dress_room_show_advance();
    }
    if (s_capture && s_frame_count >= 12) {
...
int main(int argc, char **argv) {
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--capture") == 0 && i + 1 < argc) {
            ...
            s_capture = true;
        } else if (strcmp(argv[i], "--capture-show") == 0) {
            s_capture_show = true; /* with --capture: land on Fake Show podium */
...
#define GAME_WINDOW_TITLE "Template" // CMake sets "<GAME_TITLE> [AI]" for DevAPI builds
    ...
    // Mesh stack kept for pack/shader readiness; no demo cubes in dress-up M0.
