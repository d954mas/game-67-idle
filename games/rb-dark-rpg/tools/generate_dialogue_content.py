...
def main() -> int:
    ...
    for quest in quests.values():
        ...
        lines.append(f"static const game_quest_step_definition_t QUEST_{quest_symbol}_STEPS[] = {{")
        for step in quest.get("steps", []):
            lines.append("    {")
            lines.append(f"        .id = {c_str(step.get('id'))},")
            lines.append(f"        .title = {c_str(step.get('title'))},")
            lines.append(f"        .description = {c_str(step.get('description'))},")
            lines.append(f"        .location_id = {c_str(step.get('location_id'))},")
            lines.append("    },")
        lines.append("};")
        lines.append("")

    lines.append("static const game_quest_definition_t QUESTS[] = {")
    for quest in quests.values():
        quest_symbol = c_ident(quest.get("id", "quest"))
    ...
    for encounter in encounters:
        ...
        lines.append(f"        .repeatable = {'true' if encounter.get('repeatable') else 'false'},")
        lines.append("    },")
    lines.append("};")
    lines.append("")
    ...
    lines.append("}")
    lines.append("")
    lines.append("const game_quest_definition_t *game_content_find_quest(const char *quest_id) {")
    lines.append("    if (!quest_id) {")
    lines.append("        return NULL;")
    lines.append("    }")
    lines.append("    for (int i = 0; i < (int)(sizeof QUESTS / sizeof QUESTS[0]); ++i) {")
    lines.append("        if (strcmp(QUESTS[i].id, quest_id) == 0) {")
    lines.append("            return &QUESTS[i];")
    lines.append("        }")
    lines.append("    }")
    lines.append("    return NULL;")
    lines.append("}")
    lines.append("")
    lines.append("int game_content_quest_count(void) {")
    lines.append("    return (int)(sizeof QUESTS / sizeof QUESTS[0]);")
    lines.append("}")
    lines.append("")
    lines.append("const game_quest_definition_t *game_content_quest_at(int index) {")
    lines.append("    if (index < 0 || index >= game_content_quest_count()) {")
    lines.append("        return NULL;")
    lines.append("    }")
    lines.append("    return &QUESTS[index];")
    lines.append("}")
    lines.append("")
    lines.append("const game_quest_step_definition_t *game_content_find_quest_step(const char *quest_id, const char *step_id) {")
    lines.append("    const game_quest_definition_t *quest = game_content_find_quest(quest_id);")
    lines.append("    if (!quest || !step_id) {")
    lines.append("        return NULL;")
    lines.append("    }")
    lines.append("    for (int i = 0; i < quest->step_count; ++i) {")
    lines.append("        if (strcmp(quest->steps[i].id, step_id) == 0) {")
    lines.append("            return &quest->steps[i];")
    lines.append("        }")
    lines.append("    }")
    lines.append("    return NULL;")
    lines.append("}")
