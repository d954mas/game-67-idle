...
function buildFixtureRepo() {
  ...
  writeFileSync(join(root, "templates", "template", "content", "items.lock.json"), JSON.stringify({
    ...
    receipt: {
      ...
      field_ids: { active: ["game.weapon.level.attack"], reserved: ["game.weapon.level.legacy"] },
    },
...
test("new_game --visibility public copies template and registers game assets in AI Studio", (t) => {
  ...
  assert.equal(itemsLock.schema_version, 4);
  assert.deepEqual(itemsLock.def_ids, {});
  assert.equal(itemsLock.receipt.schema, "items.release_receipt.v2");
  assert.deepEqual(itemsLock.receipt.field_ids, {
    active: ["game.weapon.level.attack"], reserved: ["game.weapon.level.legacy"],
  });
