...
@'
<packet text>
'@ | node tools/ai.mjs subagent-packet-check --stdin --json
```

Use the check when the packet is stored in a scratch file, generated by another
tool, or pasted as a shell here-string. For quick inline prompts, copy the
template shape directly and keep every field visible in the prompt.
