...
## Пост-ратификация: лог-зеркало событий (решение лида 2026-07-07)

- **Что:** тюнабл `GAME_EVENTS_LOG_MIRROR` (дефолт OFF) в `game_events_devapi.c`; при `=1`
  каждое уже отрендеренное событие кадра дополнительно пишется в движковый лог строкой
  `[ev] <полная JSON-строка рендерера>`.
- **Где:** рекордер `game_events_devapi_record()` — сразу после `game_event_render` в
  `s_ring[s_ring_head]`, под `#if GAME_EVENTS_LOG_MIRROR` (`nt_log_info("[ev] %s", ...)`).
  Реюз готовой ring-строки → НОЛЬ дополнительного рендера. Дефайн — рядом с прочими тюнаблами
  TU (`#ifndef … #define 0 #endif`).
- **Как включить:** `-DGAME_EVENTS_LOG_MIRROR=1` через `target_compile_definitions` в конфиге
  игры (CMake-опция НЕ добавляется). Доступно в devapi-сборках (TU под `NT_DEVAPI_ENABLED`,
  где логи и читают). Формат безопасен под `-Wformat=2` (литеральный формат, `%s` над
  NUL-терминированной валидной JSON-строкой).
