...
const OWN_STUDIO_ROOT = findStudioRoot(fileURLToPath(new URL("..", import.meta.url)));
const { inspectPlatformSdkArtifact } = await import(
  pathToFileURL(join(OWN_STUDIO_ROOT, "features", "platform-sdk", "scripts", "artifact_tools.mjs")).href
);
...
export function validateWebArtifact({ artifactDir, target, studioRoot = OWN_STUDIO_ROOT }) {
...
export function verifyWebPackage({ zipPath, manifestPath, expectedTarget, studioRoot = OWN_STUDIO_ROOT }) {
