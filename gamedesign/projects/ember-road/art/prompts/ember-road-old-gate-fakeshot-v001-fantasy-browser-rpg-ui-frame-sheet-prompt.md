...
- composed_old_gate_fake_shot (reference_image): single composed target for the first native screen: Old Gate scene, hero, Gate Warden quest focus, route strip, wolf encounter preview, reward preview, and persistent browser-RPG frame
- old_gate_background_layers (background): painted Old Gate town-hub backdrop and route strip layers
- hero_npc_enemy_sprites (sprite): isolated hero, Gate Warden, and Road Wolf silhouettes with pivots for runtime composition
- quest_reward_route_icons (icon): quest marker, route marker, lock, wolf encounter marker, ring reward, XP, and gold icons
- fantasy_browser_rpg_frame (slice9): ornate but readable browser-RPG frame pieces for top hero status, right quest rail, bottom log/action belt, button bases, and reward slots
