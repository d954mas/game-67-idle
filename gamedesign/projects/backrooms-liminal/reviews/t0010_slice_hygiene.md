...
- profiler guard: node tools/ai.mjs status --require-current-scope-usable: usable current scope T0010/opaque-native-portal-pass (unknown)
- known red gates: T0010 product gate intentionally remains FAIL for art_quality/audience_fit until the portal interior is fully native opaque geometry or T0011 render-target portal lighting replaces the hybrid composite

## Warnings
- profiler guard evidence is inconclusive (advisory): node tools/ai.mjs status --require-current-scope-usable: usable current scope T0010/opaque-native-portal-pass (could not confirm usable current-scope profiler guard)
