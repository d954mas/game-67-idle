...
def render_abi_header(snapshot: dict[str, Any]) -> str:
    ...
    for item in sorted_items:
        ...
        item_records.append((item_id, macro, digest))
    field_records = []
    ...
    lines.extend([
        ...
        "    uint32_t type;",
        "    uint32_t rounding;",
        "} items_catalog_expected_field_t;",
        "",
        "static const char *const s_items_catalog_expected_item_ids[",
        "    ITEMS_CATALOG_ITEM_COUNT > 0U ? ITEMS_CATALOG_ITEM_COUNT : 1U] = {",
    ])
    lines.extend(f"    {json.dumps(item_id)}," for item_id, _, _ in item_records)
    if not item_records:
        lines.append('    "",')
    lines.extend([
        "};",
        "static const items_catalog_expected_field_t s_items_catalog_expected_fields[",
        "    ITEMS_CATALOG_FIELD_COUNT > 0U ? ITEMS_CATALOG_FIELD_COUNT : 1U] = {",
    ])
    ...
    for field_id, member, unit, minimum, maximum, _ in field_records:
        min_c = c_i64(minimum)
        max_c = c_i64(maximum)
        lines.append(
            f"    {{ {json.dumps(field_id)}, {json.dumps(member)}, {json.dumps(unit)}, "
            f"{min_c}, {max_c}, UINT32_C(1), UINT32_C(1) }},"
        )
