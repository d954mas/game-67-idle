...
def render_source(table: Table) -> str:
    ...
    lines.append("")
    lines.append("/* One bit per key, set the first time that key is RESOLVED FOR DISPLAY.")
    lines.append("   Every gate in this module proves the text is WIRED; this is the only")
    lines.append("   thing that can say whether anyone ever had the chance to READ it, and a")
    lines.append("   key no surface reaches is a finding about the game. See")
    lines.append("   loc_key_was_seen() in loc.h. */")
    lines.append("static uint8_t s_loc_seen[(LOC_KEY_COUNT + 7) / 8];")
    lines.append("")
    ...
    lines.append("    s_loc_seen,")
    lines.append("    sizeof s_loc_seen,")
    lines.append("};")
    lines.append("")
