...
/* То же самое, но вызывающему МОЖНО ждать.
   ...
   Разделение по вызывающему, не по слоту (T0055, лид 2026-08-06): загрузка
   ...
   Эту зовут синхронные вызывающие, у которых кадра нет: загрузка, старт новой
   игры, восстановление из .bak, карантин, явный flush. Они ждут до ~0.5 c — на
   старте это невидимо, а альтернатива в том, что игрок доигрывает сессию с
   неперезаписанным битым primary.

   В РЕЛИЗНОЙ нативной сборке ожидание в кадр не попадает вообще: у
   game_save_flush всего два вызывающих — dev-API endpoint (только dev-сборки) и
   веб-flush на pagehide, а веб-бэкенд may_wait игнорирует. */
