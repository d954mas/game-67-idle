...
/* Re-run fragment reconciliation from `id` through the rest of the registry.
   Raw tooling edits publish one fragment without going through load(), so the
   game composition layer calls this before rebinding its live consumers.
   Registration order remains the single dependency order. */
...
typedef enum {
    ...
    GAME_SAVE_DEVAPI_REPLACE,
} game_save_devapi_change_t;
typedef void (*game_save_devapi_change_fn)(
...
/* Регистрирует 7 команд game.state.* над реестром фрагментов (A5; заменяет
   ...
   features/game-state/src/game_save_devapi.c. */
...
typedef enum {
    ...
    GAME_SAVE_LOAD_BLOCKED,        /* primary не читается/не карантинится -> НОЛЬ мутаций и записей */
...
/* Кадровый тик: сейв при (now-dirty_at >= DEBOUNCE) || (now-last_save >= MAX_INTERVAL).
   No-op пока не dirty или автосейв на паузе
...
/* Доходит ли прогресс игрока до диска — ОДИН вопрос, один ответ.
   ...
   «всё хорошо», отказывая при этом в КАЖДОЙ записи. */
typedef enum {
