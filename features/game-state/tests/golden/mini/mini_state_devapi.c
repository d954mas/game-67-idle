...
/* err already carries a message snprintf'd into s_state_err by a mini_state_* call. */
...
/* mini_state_* helpers return freshly-built cJSON; the engine ABI fills a pre-created
...
static bool check_doc(const cJSON *params, nt_devapi_error *err) {
    ...
    if (!doc || (cJSON_IsString(doc) && strcmp(doc->valuestring, "mini") == 0)) {
        return true;
    }
...
static bool ep_state_schema(const cJSON *params, cJSON *result_obj, nt_devapi_error *err, void *user) {
    ...
    return state_emit(result_obj, mini_state_schema_json(), err);
}
...
static bool ep_state_get(const cJSON *params, cJSON *result_obj, nt_devapi_error *err, void *user) {
    ...
    cJSON *value = mini_state_get_path_json(&mini_state, state_path, s_state_err, (int)sizeof(s_state_err));
    if (!value) {
...
static bool ep_state_set(const cJSON *params, cJSON *result_obj, nt_devapi_error *err, void *user) {
    ...
    if (!mini_state_set_path_json(&mini_state, path->valuestring, value, s_state_err, (int)sizeof(s_state_err))) {
        return state_fail_buf(err, "bad_params");
    }
    game_save_mark_dirty();
    return state_emit(result_obj, mini_state_to_json(&mini_state), err);
}
...
static bool ep_state_patch(const cJSON *params, cJSON *result_obj, nt_devapi_error *err, void *user) {
    ...
    if (!mini_state_patch_json(&mini_state, values, s_state_err, (int)sizeof(s_state_err))) {
        return state_fail_buf(err, "bad_params");
    }
    game_save_mark_dirty();
    return state_emit(result_obj, mini_state_to_json(&mini_state), err);
...
static bool ep_state_reset(const cJSON *params, cJSON *result_obj, nt_devapi_error *err, void *user) {
    (void)user;
    if (!check_doc(params, err)) { return false; }
    mini_state_init_defaults(&mini_state);
    game_save_mark_dirty();
    return state_emit(result_obj, mini_state_to_json(&mini_state), err);
}
...
void mini_state_register_devapi(void) {
    static const nt_devapi_command_desc descs[] = {
        {"mini.state.schema", "mini", "Return the mini state JSON schema.", "doc?", "schema object", "immediate", "none"},
        {"mini.state.get", "mini", "Get a state value by path.", "doc?, path", "path, value", "immediate", "none"},
        {"mini.state.set", "mini", "Set a state value by path.", "doc?, path, value", "state object", "immediate", "mutates state"},
        {"mini.state.patch", "mini", "Patch multiple state values.", "doc?, values", "state object", "immediate", "mutates state"},
        {"mini.state.save", "mini", "Flush state to storage.", "doc?", "saved", "immediate", "writes file"},
        {"mini.state.load", "mini", "Reload state from storage.", "doc?", "state object", "immediate", "mutates state"},
        {"mini.state.reset", "mini", "Reset state to defaults.", "doc?", "state object", "immediate", "mutates state"},
    };
