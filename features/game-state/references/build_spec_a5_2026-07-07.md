...
- [ ] ПРИМЕЧАНИЕ (Q1, ждёт решения лида): `get {path:""}` НАМЕРЕННО ≠ on-disk features-map
      — orphan-ключи (retained verbatim по §14 п.16) в DevAPI-агрегат НЕ входят. Это
      осознанное расхождение с §8 «агрегат = features-map» (см. §A5.14 Q1); при
      подтверждении обратного — тривиальный реверс.
...
- **Q1 [ДЕФОЛТ ПРИМЕНЁН, ЖДЁТ ПОДТВЕРЖДЕНИЯ ЛИДА]. Включать ли orphan-блобы (retained
  незнакомые фрагмент-ключи) в DevAPI-агрегат `get ""`/`schema`?** ДЕФОЛТ: НЕТ — агрегат
