...
def main() -> int:
    ...
    try:
        with running_game(port=PORT) as bus:
            ...
            ok &= check("set wallet", state.get("wallet", {}).get("soft") == 1234, state)
            ...
            reset_wallet = bus.result("game.state.get", {"doc": "game", "path": "wallet.soft"})
            ok &= check("reset wallet", reset_wallet == 0, reset_wallet)
            loaded = bus.result("game.state.load", {"key": save_key, "doc": "game"})
            ok &= check("load restored wallet", loaded.get("wallet", {}).get("soft") == 1234, loaded)
