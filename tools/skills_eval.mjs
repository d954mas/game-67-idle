...
const SKILL_CHECKS = [
  ...
  },
  {
    ...
    sourceDir: "ai_studio/core_harness/profiling/skills/nt-chat-session-reflection",
    frontmatter: [
      "full retrospective",
      "AI-assisted work session",
      "chat history",
      "agent run",
      "profiling data",
      "slow progress",
      "repeated failures",
      "weak tool/subagent use",
      "process improvements",
      "comprehensive evidence-based session review",
    ],
    body: [
      "full retrospective",
...
for (const check of SKILL_CHECKS) {
  ...
  const contentDir = check.sourceDir ? join(root, check.sourceDir) : skillDir;
  const contentFile = join(contentDir, "SKILL.md");
  const contentText = existsSync(contentFile) ? readFileSync(contentFile, "utf8") : body;
  const content = splitFrontmatter(contentText).body;
  const bodyWithReferences = `${body}\n${content}\n${readReferenceBodies(contentDir)}`;
