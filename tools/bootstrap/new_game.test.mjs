...
test("new_game copies template and registers game assets in AI Studio", (t) => {
  ...
  assert.match(output, /registered assets: ai_studio\/assets\/assets_storage\/source_registry\/games\.json -> test-game\/assets/);
  ...
  const registryPath = join(root, "ai_studio", "assets", "assets_storage", "source_registry", "games.json");
...
test("new_game --force updates the same registry entry", (t) => {
  const root = tempRepo();
  t.after(() => rmSync(root, { recursive: true, force: true }));
  mkdirSync(dirname(join(root, "ai_studio", "assets", "assets_storage", "source_registry", "games.json")), { recursive: true });
  writeFileSync(join(root, "ai_studio", "assets", "assets_storage", "source_registry", "games.json"), JSON.stringify({
  ...
  const registry = JSON.parse(readFileSync(join(root, "ai_studio", "assets", "assets_storage", "source_registry", "games.json"), "utf8"));
  assert.deepEqual(registry.games, [{
    id: "test-game",
    title: "test-game",
    folder: "test-game",
