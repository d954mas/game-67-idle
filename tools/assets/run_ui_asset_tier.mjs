...
function usage() {
  console.log(`usage: node tools/assets/run_ui_asset_tier.mjs --tier draft|integrate|final --plan \\
...
function planFor(tier, vars) {
  ...
  if (tier === "draft") {
    return [
      `py -3.12 tools/assets/audit_source_sheet_intake.py --source ${sheet} --json-output tmp/intake.json --report tmp/intake.md`,
      `py -3.12 tools/assets/build_runtime_assets_from_crop_plan.py --crop-plan <crop-plan.json> --crop-manifest ${crop} --asset-manifest ${runtime} --art-job ${job} --contact-sheet tmp/contact.png`,
    ];
  }
  ...
  if (tier === "integrate") {
    return [
      ...
      `py -3.12 tools/assets/audit_generated_ui_assets.py --crop-manifest ${crop} --json-output tmp/pixel-audit.json --report tmp/pixel-audit.md`,
      `py -3.12 tools/assets/render_ui_composition_proof.py --asset-manifest ${runtime} --output tmp/proof.png --json-output tmp/proof.json --report tmp/proof.md`,
    ];
  }
  ...
  return [
    ...
    `node tools/assets/validate_art_job.mjs --job ${job} --strict`,
    `py -3.12 tools/assets/audit_generated_ui_assets.py --crop-manifest ${crop} --json-output tmp/pixel-audit.json --report tmp/pixel-audit.md`,
    `py -3.12 tools/assets/render_ui_asset_edge_proof.py --crop-manifest ${crop} --output tmp/edge-proof.png --json-output tmp/edge-proof.json --report tmp/edge-proof.md --only-problems`,
    `node tools/assets/audit_slice9_design_policy.mjs --crop-manifest ${crop} --runtime-manifest ${runtime} --json-output tmp/slice9.json --report tmp/slice9.md`,
    `py -3.12 tools/assets/render_ui_composition_proof.py --asset-manifest ${runtime} --output tmp/proof.png --json-output tmp/proof.json --report tmp/proof.md`,
    `node tools/assets/audit_atlas_metadata.mjs --asset-manifest ${runtime} --json-output tmp/atlas-meta.json --report tmp/atlas-meta.md`,
    `py -3.12 tools/assets/build_ui_atlas_pack.py --asset-manifest ${runtime} --output-dir tmp/review-atlas --json-output tmp/atlas-pack.json --report tmp/atlas-pack.md --label-review`,
    `py -3.12 tools/assets/audit_ui_atlas_pack.py --atlas-pack tmp/atlas-pack.json --asset-manifest ${runtime} --json-output tmp/atlas-audit.json --report tmp/atlas-audit.md`,
    `node tools/assets/audit_source_family_coverage.mjs --job ${job} --json-output tmp/family-coverage.json --report tmp/family-coverage.md`,
    `py -3.12 tools/assets/audit_generated_source_derivation.py --crop-manifest ${crop} --json-output tmp/derivation.json --report tmp/derivation.md`,
    `node tools/assets/audit_runtime_ui_asset_usage.mjs --asset-manifest ${runtime} --usage <runtime-usage.json> --json-output tmp/usage.json --report tmp/usage.md`,
