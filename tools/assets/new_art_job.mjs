...
//
// Example:
//   node tools/assets/new_art_job.mjs --id 67-world-characters-v2 --family "next 67 variants" --concept meme-evolution --target gamedesign/projects/meme-evolution/visuals/67-world-first-7-lineup-v1.png
//
...
function usage() {
  console.log("usage: node tools/assets/new_art_job.mjs --id <job-id> --family <asset family> [--concept meme-evolution] [--project-dir gamedesign/projects/meme-evolution] [--target path] [--audience text] [--dry-run]");
}
...
const artJob = {
  ...
  commands: {
    plan_source_sheet_prompt: `node tools/assets/plan_source_sheet_prompt.mjs --job ${artRequestPath} --source-family "<source family>" --output ${projectDir}/art/prompts/<source-id>-prompt.md --json-output ${projectDir}/art/prompts/<source-id>-prompt.json`,
    new_generation_record: `node tools/assets/new_generation_record.mjs --id <source-id> --project-dir ${projectDir} --source-family "<source family>" --source-family-role "<source family role>" --accepted-source <path> --provider <provider> --model <model-or-workflow> --workflow-path <workflow.json> --prompt-packet ${projectDir}/art/prompts/<source-id>-prompt.json --seed <seed> --prompt "<prompt>" --negative-prompt "<negative prompt>"`,
    slice_assets: "fill after source sheets are accepted",
    validate_draft: `node tools/assets/validate_art_job.mjs --job ${artRequestPath}`,
    validate_strict: `node tools/assets/validate_art_job.mjs --job ${artRequestPath} --strict`,
    validate_final_art: `node tools/assets/validate_art_job.mjs --job ${artRequestPath} --final-art`,
    asset_audit: `py -3.12 tools/assets/audit_generated_ui_assets.py --crop-manifest ${cropManifestPath} --json-output ${auditJsonPath} --report ${auditMarkdownPath}`,
    slice9_design_audit: `node tools/assets/audit_slice9_design_policy.mjs --crop-manifest ${cropManifestPath} --runtime-manifest ${runtimeManifestPath} --json-output ${slice9DesignAuditJsonPath} --report ${slice9DesignAuditMarkdownPath}`,
    source_family_coverage_audit: `node tools/assets/audit_source_family_coverage.mjs --job ${artRequestPath} --json-output ${sourceFamilyCoverageJsonPath} --report ${sourceFamilyCoverageMarkdownPath}`,
    plan_missing_source_family_prompts: `node tools/assets/plan_missing_source_family_prompts.mjs --job ${artRequestPath} --coverage-audit ${sourceFamilyCoverageJsonPath} --output-dir ${projectDir}/art/prompts --key-color "#00ff00" --force`,
