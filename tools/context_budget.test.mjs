...
test("context budget default hot doc limit is tight enough for live docs", () => {
  ...
  try {
    ...
    assert.equal(result.status, 1);
    assert.match(result.stderr, /AGENTS\.md: 6600 chars > 3600/);
  } finally {
    cleanup(dir);
  }
});
...
test("context budget default skill entrypoint cap stays tight", () => {
  const dir = tempDir();
  try {
    writeFixture(dir, "x".repeat(3900));
    const result = run(["--root", dir]);
    assert.equal(result.status, 1);
    assert.match(result.stderr, /\.codex\/skills\/sample\/SKILL\.md: 3900 chars > 3800/);
  } finally {
    cleanup(dir);
  }
});
