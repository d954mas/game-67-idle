# Agent Docs

This folder contains project-specific operational memory for AI agents working on Game 67.

It is not the GDD, not a replacement for skills, and not a general AI workflow manual.

Before non-trivial work:

1. Read `AGENTS.md`.
2. Read `agent_docs/project_state.md`.
3. Use the relevant reusable project skills for general workflow rules.

Reusable workflow belongs in `.codex/skills/`.
Project-specific facts, commands, constraints, current state, and repo traps belong in `agent_docs/project_state.md`.

Update `agent_docs/project_state.md` after meaningful verified iterations, target changes, or newly discovered repo traps.
