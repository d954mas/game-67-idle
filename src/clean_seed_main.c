...
static void register_game_endpoints(void) {
    static const nt_devapi_command_desc descs[] = {
        {"game.state", "game", "Return clean seed runtime state.", "", "{state}", "immediate", "none"},
        {"game.reset_playtest", "game", "Reset clean seed state for automation.", "", "{state}", "immediate", "resets state"},
        {"game.action.cycle", "game", "Cycle the clean seed shape.", "", "{state}", "next-frame", "mutates seed state"},
    };
