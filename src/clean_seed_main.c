...
static void register_game_endpoints(void) {
    static const nt_devapi_command_desc descs[] = {
        {"game.state", "game", "Return clean seed runtime state.", "", "{runtime,shape}", "immediate", "none"},
        {"game.reset_playtest", "game", "Reset clean seed state for automation.", "", "{runtime,shape}", "immediate", "resets state"},
        {"game.action.cycle", "game", "Cycle the clean seed display shape.", "", "{runtime,shape}", "next-frame", "mutates state"},
    };
...
static void register_ui_devapi(float w, float h) {
    const float cycle_y = h - s_cycle_box.y - s_cycle_box.h;
    const float meter_y = h - (s_cycle_box.y - 34.0F) - 14.0F;
    game_devapi_ui_clear();
    (void)game_devapi_ui_register_node("root", "", "screen", "Game Seed", "Clean seed runtime", 0.0F, 0.0F, w, h, true, true);
    (void)game_devapi_ui_register_node("seed.cycle", "root", "button", "Cycle Seed", g_game_state.test_button_text, s_cycle_box.x, cycle_y, s_cycle_box.w, s_cycle_box.h, true, true);
    (void)game_devapi_ui_register_node("seed.progress", "root", "meter", "Seed Progress", g_game_state.test_label_text, w * 0.5F - 160.0F, meter_y, 320.0F, 14.0F, true, true);
}
#endif
