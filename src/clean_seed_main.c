...
static void draw_button(UiBox box) {
    ...
    const float pointer_y = h - g_nt_input.pointers[0].y;
    const bool hot = nt_input_mouse_is_down(NT_BUTTON_LEFT) && contains(box, g_nt_input.pointers[0].x, pointer_y);
...
static void draw_scene(float w, float h) {
    ...
    rect(0.0F, 0.0F, w, h * 0.38F, (float[4]){0.74F, 0.92F, 0.66F, 1.0F});
    rect(w * 0.10F, h * 0.30F, w * 0.80F, h * 0.54F, (float[4]){1.0F, 1.0F, 1.0F, 0.22F});
    rect(w * 0.10F, h * 0.82F, w * 0.80F, 8.0F, (float[4]){0.08F, 0.58F, 0.88F, 1.0F});
    draw_seed_shape(w, h);
    ...
    const float meter_y = s_cycle_box.y + s_cycle_box.h + 20.0F;
...
static void handle_input(void) {
    ...
    if (nt_input_mouse_is_pressed(NT_BUTTON_LEFT)) {
        ...
        for (int i = 0; i < NT_INPUT_MAX_POINTERS; ++i) {
            ...
            if (pointer.active && contains(s_cycle_box, pointer.x, h - pointer.y)) {
                cycle_seed();
                return;
            }
        }
...
static void register_ui_devapi(float w, float h) {
    ...
    (void)game_devapi_ui_register_node("seed.progress", "root", "meter", "Seed Progress", g_game_state.test_label_text, w * 0.5F - 160.0F, s_cycle_box.y + s_cycle_box.h + 20.0F, 320.0F, 14.0F, true, true);
}
#endif
