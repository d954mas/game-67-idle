/*
 * Build the Voxelheim runtime pack:
 *   assets/voxelheim.ntpack -- sprite + slug_text shaders, the HUD font, and a
 *   "voxels" atlas packing the 14 cut transparent PNGs in assets/raw/.
 *
 * Usage: build_voxelheim_packs <pack_dir>
 * Run from the project root (paths below are project-root relative).
 *
 * Model: examples/bunnymark/build_packs.c (engine submodule).
 */
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        (void)fprintf(stderr, "Usage: build_voxelheim_packs <pack_dir>\n");
        return 1;
    }
    const char *out_dir = argv[1];

    (void)printf("=== Build Voxelheim Pack -> %s ===\n\n", out_dir);
    ...
    NtBuilderContext *ctx = nt_builder_start_pack(pack_path(out_dir, "voxelheim.ntpack"));
    if (!ctx) {
        (void)fprintf(stderr, "Failed to start voxelheim.ntpack\n");
        return 1;
    }
    ...
    // #region shaders (sprite + slug_text, copied into assets/shaders/)
    ...
    // #region HUD font (ASCII only -- HUD text is Latin)
    (void)fprintf(stderr, "[packer] adding font...\n");
    nt_builder_add_font(ctx, "assets/fonts/voxelheim_hud.ttf",
                        &(nt_font_opts_t){
                            .charset = NT_CHARSET_ASCII,
                            .resource_name = "voxelheim/font_hud",
                        });
    (void)printf("  Font (ASCII) added: voxelheim/font_hud\n");
    (void)fprintf(stderr, "[packer] font done\n");
    // #endregion

    // #region atlas: 14 cut PNGs from assets/raw/
    ...
    atlas_opts.max_size = 4096; /* keep all 14 regions on one page */
    ...
    nt_builder_begin_atlas(ctx, "voxels", &atlas_opts);
    /* Glob picks up all 14 cut PNGs; each derives its region name from basename
     * (e.g. assets/raw/hero.png -> voxels/hero.png). opts MUST be NULL for glob;
     * centre pivot (0.5, 0.5) applies by default. */
    nt_builder_atlas_add_glob(ctx, "assets/raw/*.png", NULL);
    nt_builder_end_atlas(ctx);
    (void)printf("  Atlas 'voxels' added: assets/raw/*.png (RECT, ppu=1.0)\n");
    // #endregion
    ...
    if (r != NT_BUILD_OK) {
        (void)fprintf(stderr, "voxelheim.ntpack failed: %d\n", r);
        return 1;
    }

    char base_hdr[512];
    (void)snprintf(base_hdr, sizeof(base_hdr), "%s/voxelheim.h", HEADER_DIR);
    const char *headers[] = {base_hdr};
    char combined[512];
    (void)snprintf(combined, sizeof(combined), "%s/voxelheim_assets.h", HEADER_DIR);
    nt_builder_merge_headers(headers, 1, combined);
    (void)printf("Generated: %s\n", combined);
    // #endregion

    (void)printf("\n=== Pack Size Summary ===\n");
    FILE *f = fopen(pack_path(out_dir, "voxelheim.ntpack"), "rb");
    if (f) {
        (void)fseek(f, 0, SEEK_END);
        long sz = ftell(f);
        (void)fclose(f);
        (void)printf("  voxelheim.ntpack    %8.1f KB\n", (double)sz / 1024.0);
    }
