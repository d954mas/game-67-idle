...
# acceptance gate (ai_studio/assets/tools/image/alpha_dualplate/dual_plate_pair_gate.py) checks the pair really is
...
GATE="$REPO_ROOT/ai_studio/assets/tools/image/alpha_dualplate/dual_plate_pair_gate.py"
...
if [ "$GATE_RC" -eq 0 ]; then
  echo "PAIR OK -> extract: py -3.12 ai_studio/assets/tools/image/alpha_dualplate/dual_plate_alpha.py --light '$WHITE' --dark '$BLACK' --output '$OUTDIR/${NAME}_rgba.png' --alpha-combine proj"
else
